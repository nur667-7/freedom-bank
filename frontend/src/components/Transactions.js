...
const fmt = (n) =>
  n ? `${Number(n).toLocaleString("ru-KZ")} ₸` : "—";
...
export default function Transactions() {
  ...
  const handleAdd = async () => {
    ...
    if (!acc) {
      showToast("Счёт не найден", "error");
      return;
    }
  ...
  const getAccountNum = (t) => {
    ...
    const acc = accounts.find(
      (a) => a.accountId === (t.account.accountId || t.account)
    );
      ...
      <div className="section-header">
        <div>
          <h1 className="section-title">
            Транзакции <span style={{ color: "var(--purple)" }}>●</span>
          </h1>
          <p className="section-sub">
            Все операции · {transactions.length} записей
          </p>
        </div>
      ...
      <div className="table-wrap">
        ...
        {filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="et">Транзакций нет</div>
          </div>
        ) : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  set("transactionType", e.target.value)
                }
              >
